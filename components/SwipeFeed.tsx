...
export default function SwipeFeed({ priorityPostId }: { priorityPostId?: string }) {
  ...
  const remaining = useMemo(() => {
    ...
    if (!hasHistory && cfScores.size === 0) {
      sorted = [...unseen].sort((a, b) => Number(b.is_boosted) - Number(a.is_boosted));
    } else {
      sorted = [...unseen].sort((a, b) => {
    ...
    // シェアリンクから来た場合は指定ポストを先頭に
    if (priorityPostId) {
      const idx = sorted.findIndex((v) => v.id === priorityPostId);
      if (idx > 0) {
    ...
    return sorted;
  }, [seenIds, likedIds, weights, hasHistory, cfScores, vlivers, priorityPostId]);
